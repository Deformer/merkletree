...
// ReceiveHash function calculate ahsh of node content
// if content is nil then we are using precalculated Hash (mostly in case when node is proof node)
func (n *Node) ReceiveHash() ([]byte, error) {
	if n.C == nil {
		if n.Hash == nil {
			return nil, errors.New("node with nil Hash and Content")
		}
		return n.Hash, nil
	}
...
// buildHash walks down the tree until hitting a leaf, calculating the hash at each level
// and returning the resulting hash of Node n.
func (n *Node) buildHash(sort bool) ([]byte, error) {
	if n.Leaf() {
		return n.ReceiveHash()
	}
	rightBytes, err := n.Right.buildHash(sort)
	if err != nil {
		return nil, err
	}

	leftBytes, err := n.Left.buildHash(sort)
	if err != nil {
		return nil, err
	}
...
func (m *MerkleTree) VerifyTree() (bool, error) {
	calculatedMerkleRoot, err := m.Root.buildHash(m.sort)
	if err != nil {
		return false, err
	}
...
// CalculateMerkleRoot calculating MerkleRoot starting from head node
func (m *MerkleTree) CalculateMerkleRoot() ([]byte, error) {
	calculatedMerkleRoot, err := m.Root.buildHash(m.sort)
	if err != nil {
		return nil, err
	}
	return calculatedMerkleRoot, nil
}
...
func (m *MerkleTree) VerifyContent(content Content) (bool, error) {
	for _, l := range m.Leafs {
		...
		if ok {
			...
			for currentParent != nil {
				rebuiltHash, err := currentParent.buildHash(m.sort)
				if err != nil {
					return false, err
				}
				if bytes.Compare(rebuiltHash, currentParent.Hash) != 0 {
					return false, nil
				}
				currentParent = currentParent.Parent
			}
...
func (n *Node) String() string {
	return fmt.Sprintf("%t %t %v %s", n.Leaf(), n.Duplicate(), n.Hash, n.C)
}
